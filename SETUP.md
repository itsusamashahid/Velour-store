...
2. Name it (e.g. "roadkit-store"), set a database password (save it somewhere safe), pick a region close to Pakistan (e.g. Singapore), click **Create**
...
1. Go to **github.com** → sign up if needed → **New repository** → name it `roadkit-store` → **Create**
...
2. **Add New → Project** → select `roadkit-store` → **Import**
3. Before deploying, open **Environment Variables** and add:
   - `NEXT_PUBLIC_SUPABASE_URL` → your Supabase Project URL
   - `NEXT_PUBLIC_SUPABASE_ANON_KEY` → your Supabase anon public key
4. Click **Deploy**. You'll get a live URL like `roadkit-store.vercel.app`.
...
1. Buy a domain (e.g. Namecheap, GoDaddy) — something like `roadkit.pk`
